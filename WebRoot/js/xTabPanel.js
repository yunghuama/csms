...
TabPanel.prototype = {
  ...
  show: function(tabId){
    ...
    //是否已经显示
    if (this.active === tabId && !this.$tabs[tabId].htmlChanged) {
      return;
    }
    //判断是否有该ID的卡
    if (this.$tabs[tabId]) {
    ...
    //  }
      this.$tabs[tabId].htmlChanged = false;
    } else {
      alert('ID not found.');
    }
    this.moveToVisible();
   ...
   * @param {boolean}         html
   */
  setHtml: function(tabId, html){
    tabId = this._getItemId(tabId);
    this.$tabs[tabId].content.html('');
    this.$tabs[tabId].html = html;
    this.$tabs[tabId].htmlChanged = true;
    if(this.active === tabId) {
      this.show(tabId);
    }
  },
  ...
  flush : function(tabId) {
    ...
    if(!this.$tabs[tabId]) {
      ...
      if(iframes.length > 0) {
        ...
        if(iframeObj == undefined){//如果没有子iframe，则不执行更新
        	return false;
        }
